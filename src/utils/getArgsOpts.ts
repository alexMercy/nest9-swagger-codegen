...
import { argsOptions, generateOptionExpectedValues } from "./constants";
...
const getArgsOpts = (args: string[]) => {
   ...
   */
  const isGenerateOptions = args.findIndex(arg => arg === argsOptions.GENERATE_OPTIONS || arg === argsOptions.GENERATE_OPTIONS_SHORT)
  ...
  if (isGenerateOptions + 1 && generateOptionsInStr) {
    ...
    const optionsToProcess: string[] = _.intersection(generateOptionsInArray, generateOptionExpectedValues)
